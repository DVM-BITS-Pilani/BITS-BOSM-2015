...
def participant_sheet(request):
  ...
  for u in us:
    ...
    group=u.gleader
    if group is not None and group.initialregistration_set.exists():
...
@staff_member_required
def coach_sheet(request):
  book = xlwt.Workbook()
  sheet = book.add_sheet('Coach_sheet_full')
  style = xlwt.easyxf('font: name Sans-Serif, color-index blue, bold on')
  sheet.write(0,0,'Name',style = style)
  sheet.write(0,1,'Phone',style = style)
  sheet.write(0,2,'College',style = style)
  cset=Participant.objects.filter(coach=True)
  row=1
  for c in cset:
    if c.gleader is None or not c.gleader.initialregistration_set.exists():
      continue
    sheet.write(row,0,c.name)
    sheet.write(row,1,c.phone)
    sheet.write(row,2,c.gleader.initialregistration_set.all()[0].college)
    row+=1
  response = HttpResponse(mimetype='application/vnd.ms-excel')
  response['Content-Disposition'] = 'attachment; filename=Coach_all.xls'
  book.save(response)
  return response
